...
export default function EventsEdit() {
  ...
  const { isLoading, isError } = useGetData({
    ...
    params: {
      ...
      onSuccess: (data: any) => {
        ...
        const teamIds = Array.isArray(event.team_user_ids)
          ? event.team_user_ids
          : [];

        form.reset({
          event_datetime: toDatetimeLocalValue(event.event_datetime),
          team_user_ids: teamIds,
        });
        setSelectedTeamUserIds(teamIds);

        const nextParticipants = Array.isArray(event.participants)
  ...
  if (isError) {
    return <div className="p-6">Error fetching event data. Please try again.</div>;
  }
      ...
      <div className="flex items-center justify-between w-full">
        ...
        <div className="flex-1 mr-9 text-center">
          <div className="-ml-4">
            ...
            <p className="text-xs mb-9">Update the event details.</p>
          </div>
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                      ...
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-right">
                          {participants.length > 1 ? (
                            <Button type="button" variant="ghost" onClick={() => removeParticipantRow(index)}>
                              Remove
                            </Button>
                          ) : null}
                        </TableCell>
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button type="submit" className="hover:pointer" disabled={updateEvent.isPending}>
              {updateEvent.isPending ? "Saving..." : "Submit"}
            </Button>
          </div>
        </form>
      </Form>
