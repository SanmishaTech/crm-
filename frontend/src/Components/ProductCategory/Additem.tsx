...
const AddItem: React.FC<AddItemProps> = ({ onAdd, typeofschema, add }) => {
  ...
  const handleAdd = async () => {
    setLoading(true);
    try {
      await axios.post(`/api/product_categories`, formData, {
        headers: {
      ...
      setError("Failed to add product_category: " + err.message);
    }
  };
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the details of the Product Category you want to add to the order.
          </DialogDescription>
        </DialogHeader>
