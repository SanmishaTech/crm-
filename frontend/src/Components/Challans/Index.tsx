...
export default function TableDemo() {
  ...
  const { data: Sup } = useGetData({
    endpoint: `/api/challans?search=${searchTerm}&page=${currentPage}&total=${totalPages}`,
    params: {
      ...
      onError: (error) => {
        if (error.message && error.message.includes("Duplicate Challans")) {
          toast.error(
            "Challan name is Duplicated. Please use a unique name."
          );
        } else {
          ...
          <Table>
            ...
            <TableHeader>
              <TableRow className="hover:bg-accent/50">
                <TableHead className="text-foreground">Challan Number</TableHead>
