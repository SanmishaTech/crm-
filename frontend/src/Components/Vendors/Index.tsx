...
const formSchema = z.object({
  vendor_name: z.string().min(2).max(50),
  gstin: z.string().min(2).max(50),
  contact_name: z.string().min(2).max(50),
  email: z.string().email(),
  mobile_1: z.string().min(2).max(50),
  mobile_2: z.string().min(2).max(50),
...
export default function TableDemo() {
  ...
  const { data: Sup } = useGetData({
    endpoint: `/api/vendors?search=${searchTerm}&page=${currentPage}&total=${totalPages}`,
    params: {
      queryKey: ["vendors", searchTerm, currentPage],
      retry: 1,
      ...
      <div className="flex-1 p-2 md:p-6 w-full bg-accent/60 md:ml-4 mr-8 rounded-lg shadow-lg">
        ...
        <div className="p-4 rounded-md bg-card">
          <Table>
            ...
            <TableBody>
              {Sup?.data?.Vendors?.map((vendor) => (
                <TableRow key={vendor.id} className="hover:bg-accent/50">
