...
export default function TableDemo() {
  ...
  const [sortConfig, setSortConfig] = useState<{ key: keyof Client | ""; direction: "asc" | "desc" }>({ key: "", direction: "asc" });

  const { searchTerm, setSearchTerm, isMinimized } = useSidebar();
    ...
    <div className="flex ">
      ...
      <div className="p-6 w-full bg-accent/60 ml-4 mr-8 rounded-lg shadow-lg ">
        ...
        <div className="flex justify-between items-center py-1 space-x-3">
          ...
          <div className="flex space-x-2">
            <ImportClientDialog onSuccess={() => queryClient.invalidateQueries({ queryKey: ["clients"] })} />
          ...
          <Table>
            ...
            <TableHeader>
              <TableRow>
                ...
                >
                  Clients {sortConfig.key === "client" && (sortConfig.direction === "asc" ? "↑" : "↓")}
                </TableHead>
                <TableHead
                  className="text-foreground cursor-pointer"
                  onClick={() => handleSort("contact_no")}
                >
                  Contact Number {sortConfig.key === "contact_no" && (sortConfig.direction === "asc" ? "↑" : "↓")}
                </TableHead>
                <TableHead
                  className="text-foreground cursor-pointer"
